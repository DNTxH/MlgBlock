...
class SWlistener implements Listener {
    ...
    public function onSignChange(SignChangeEvent $event) : void
    {
        ...
        if (!$player->isOp() || strtolower($event->getLine(0)) !== 'mb') {
            return;
        }
    ...
    public function onInteract(PlayerInteractEvent $event) : void
    {
		...
		if($arena !== null){
			if($event->getItem()->getId() == 288 && ($event->getAction() === PlayerInteractEvent::RIGHT_CLICK_BLOCK || $event->getAction() === PlayerInteractEvent::RIGHT_CLICK_AIR)){
				$player->setMotion(new Vector3(0, mt_rand(1.7, 2.0) , 0));
				$player->getInventory()->removeItem(Item::get(288, 0, 1));
			}
		}
		
        if($item->getName() === "§cRetrun to lobby"){
			if($arena !== null)
				$this->plugin->getServer()->dispatchCommand($player, "mb quit");
        } 
    }
            ...
            if ($arena !== null) {
                ...
                if ($event instanceof EntityDamageByEntityEvent && ($damager = $event->getDamager()) instanceof Player) {
                    if ($arena->inArena($damager) !== SWarena::PLAYER_PLAYING) {
                        $damager->getLevel()->broadcastLevelSoundEvent(new Vector3($damager->getX(), $damager->getY(), $damager->getZ()), LevelSoundEventPacket::SOUND_NOTE);
                        $event->setCancelled();
                        return;
                    }
                }
               // if ($this->plugin->configs["death.spectator"]) {
                    if ($entity->getHealth() <= $event->getFinalDamage()) {
						...
						$arena->closePlayer($entity, false, false);
    ...
    public function onCommand(PlayerCommandPreprocessEvent $event) : void
    {
        ...
        if ($command[0] === "/") {// hello, yes laith here who are u 
            $player = $event->getPlayer();
            if ($this->plugin->getPlayerArena($player) !== null) {

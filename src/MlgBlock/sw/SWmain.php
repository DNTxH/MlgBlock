...
use MlgBlock\sw\utils\SWeconomy;
...
class SWmain extends PluginBase {
    ...
    public function onEnable() : void
    {
        ...
        $this->configs = array_map(function($value){ return is_string($value) ? TextFormat::colorize($value) : $value; }, yaml_parse_file($this->getDataFolder() . "MlgBlock_configs.yml"));
        $this->lang = array_map([TextFormat::class, "colorize"], yaml_parse_file($this->getDataFolder() . "MlgBlock_lang.yml"));
        ...
        $this->getServer()->getCommandMap()->register($this->getName(), new SWcommands("mb", $this));
    ...
    {
        return $this->signs[$pos->x . ":" . $pos->y . ":" . $pos->z . ":" . $pos->getWorld()->getFolderName()] ?? null;
    }
    ...
    {
        $this->signs[$pos->x . ":" . $pos->y . ":" . $pos->z . ":" . $pos->getWorld()->getFolderName()] = $arena;
    }
    ...
    public function deleteAllSigns(?string $arena = null) : int
    {
        ...
        if ($arena === null) {
            ...
            foreach (array_keys($this->signs, $arena, true) as $xyzw) {
                ...
                $level = $server->getWorldByName($xyzw[3]);
        ...
        foreach ($this->signs as $xyzworld => $arena_name) {
            if ($arena_name === $arena) {
                ...
                if ($tile instanceof Sign) {
                    $tile->setText(
                        TextFormat::AQUA . "MlgBlock",
                        TextFormat::BOLD . TextFormat::YELLOW . "Solo",
    ...
    public function sendDeathMessage(Player $player) : void
    {
        ...
        $this->getServer()->broadcastMessage($message, $player->getWorld()->getPlayers());
    }
}